...
function ChatPage({
  ...
  isPopup: boolean;
  setIsShowChatModal?: (value: boolean) => void;
  ...
  const chatId = isPopup ? receiver : params.chatId;
  ...
  return (
    <>
      <section
        className={`flex-1 flex   h-full rounded w-full ${
          !isPopup && "border"
        }`}
      >
        {!isPopup && (
        ...
        {receiverID ? (
          <>
            ...
            <div className="w-full white  flex flex-col z">
              <div
                className={`border-b p-2 bg-white flex items-center overflow-auto`}
              >
                {!isPopup && (
                  <span
